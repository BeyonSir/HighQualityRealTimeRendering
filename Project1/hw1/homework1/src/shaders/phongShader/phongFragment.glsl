...
float findBlocker( sampler2D shadowMap,  vec2 uv, float zReceiver ) {
  ...
  if(shadowCount == BLOCKER_SEARCH_NUM_SAMPLES){
    return 2.0;
  }
...
float PCF(sampler2D shadowMap, vec4 coords) {
  ...
  for(int i = 0 ; i < PCF_NUM_SAMPLES ; i++)
  {
    ...
    if(currentDepth < closestDepth + 0.01) noShadowCount += 1;
  }

  float shadow = float(noShadowCount) / float(PCF_NUM_SAMPLES);
  return shadow;
}


float PCSS(sampler2D shadowMap, vec4 coords){
  //需要注意的是在第一步findBlocker()中会有一个窗口步长 filterStride，在第二步还有一个窗口步长 filterStride
  //在实现中第二个步长应该比第一个步长小一些，这样得到的结果才会比较合理
...
float useShadowMap(sampler2D shadowMap, vec4 shadowCoord){
  ...
  vec4 closestdepth_SM = texture2D(shadowMap, shadowCoord.xy);
  float closestDepth = unpack(closestdepth_SM);
